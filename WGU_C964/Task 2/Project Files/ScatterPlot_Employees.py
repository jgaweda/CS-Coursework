...
def read_altered_employee_records():
    ...
    with open('Data_Employee_Processed.csv') as csvfile:
        altered_records = []
        readCSV = csv.reader(csvfile, delimiter=',')
        for row in readCSV:
...
class Ui_Employee_Scatter_Plot(object):
    def setupUi(self, MainWindow):
        ...
        self.graphWidget.setLabel('left', 'Number of occurrences of employee names first letter values', color='orange', size=30)
        self.graphWidget.setLabel('bottom', 'Employee Names sorted by first letter in an employees name', color='orange', size=30)
        ...
        try:
            ...
            self.plot(alphabet_value, altered_employee_names, "Altered Employee Names", 'g')
        except SystemExit:
            msg = QMessageBox()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate("MainWindow",
                                      "NOTE: This graph displays the difference between employee first names if the Epsilon Delta Privacy Solution includes the first name values."))
